...
def draw_map(self, last_known):
    ...
    cv.putText(
        self.img, "50 Nautical Miles", (71, 370), cv.FONT_HERSHEY_PLAIN, 1, (0, 0, 0)
    )
    # box 1
    cv.rectangle(
        self.img,
...
def revise_target_probs(self):
    """Update area target probabilites based on search effectiveness"""
    denom = (
        self.p1 * (1 - self.sep1)
        + self.p2 * (1 - self.sep2)
        + self.p3 * (1 - self.sep3)
